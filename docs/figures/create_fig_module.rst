...
.. NOTE: If the code fragments below appear incorrect in the sections below it is probably because the code in xxx has changed, and the line number ranges in the `:lines:` directives need to be fixed.
...
The module docstring will appear at top of the :ref:`LibraryCodeAutoGeneratedDocs`
...
2. Call a module-private function :py:func:`_prep_fig_axes`:
   ...
   * a tuple of :py:obj:`matplotlib.axes.Axes` objects,
   ...
   The tuple of axes objects returned by :py:func:`_prep_fig_axes` should be given meaningful names as shown above
