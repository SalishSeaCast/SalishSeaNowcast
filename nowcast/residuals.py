...
SITES = {
    ...
    "Neah Bay": {"lat": 48.4, "lon": -124.6, "stn_no": 9443090},
    "Friday Harbor": {"lat": 48.55, "lon": -123.016667, "stn_no": 9449880},
    "Cherry Point": {
        "lat": 48.866667,
        "lon": -122.766667,
        "stn_no": 9449424,
...
def load_archived_observations(name, start_date, end_date):
    ...
    base_url = (
        "http://www.meds-sdmm.dfo-mpo.gc.ca/isdm-gdsi/twl-mne/inventory-inventaire/"
    )
    ...
    try:
        wlev_meas = pd.read_csv(
            fakefile, skiprows=7, parse_dates=[0], date_parser=dateparse_archive_obs
        )
...
def get_model_time_variables(grid_T):
    ...
    """
    time = nc_tools.timestamp(grid_T, range(grid_T.variables["time_counter"].size))
