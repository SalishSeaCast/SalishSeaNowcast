...
https://nbviewer.jupyter.org/github/SalishSeaCast/SalishSeaNowcast/blob/main/notebooks/figures/publish/TestCompareTidePredictionMaxSSH.ipynb

Development notebook for this module is
https://nbviewer.jupyter.org/github/SalishSeaCast/SalishSeaNowcast/blob/main/notebooks/figures/publish/DevelopCompareTidePredictionMaxSSH.ipynb
"""
...
def _prep_plot_data(
    ...
    shared.localize_time(
        ttide_ds, local_datetime=arrow.get(str(ssh_forecast.time.values[0])).to("local")
    )
...
def _residual_time_series_labels(
    ax_res, plot_data, theme, ylims=(-1, 1), yticks=numpy.arange(-1, 1.25, 0.25)
):
    ax_res.set_title("")
    ax_res.set_xlabel(
...
def _plot_ssh_map(ax_map, plot_data, place, theme):
    ...
    ax_map.contour(
        plot_data.max_ssh_field, contour_intervals, colors="black", linestyles="--"
    )
