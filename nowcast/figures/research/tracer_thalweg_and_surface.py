...
def make_figure(
    ...
    _surface_axes_labels(
        ax_surface, tracer_var, depth_integrated, clevels_surface,
        cbar_surface, theme
    )
    return fig
...
def _calc_clevels(plot_data):
    ...
    overlap = (
        max(
            0,
            min(percent_98_surf, percent_98_grid) -
            max(percent_2_surf, percent_2_grid)
        )
    )
    magnitude = ((percent_98_surf - percent_2_surf) +
...
def _surface_axes_labels(
    ...
    cbar_units = (
        f'{tracer_var.units}*m' if depth_integrated else f'{tracer_var.units}'
    )
