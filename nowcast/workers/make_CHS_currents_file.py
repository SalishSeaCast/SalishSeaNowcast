...
def _read_avg_unstagger_rotate(meshfilename, src_dir, ufile, vfile, run_type):
    ...
    uupper = (
        uds.vozocrtx.isel(depthu=slice(5))
        .where(mesh.umask.isel(z=slice(5)).rename({"z": "depthu"}))
        .mean("depthu")
    )
    ufull = (
        uds.vozocrtx.isel(depthu=slice(10))
        .where(mesh.umask.isel(z=slice(10)).rename({"z": "depthu"}))
        .mean("depthu")
    )
    ...
    vupper = (
        vds.vomecrty.isel(depthv=slice(5))
        .where(mesh.vmask.isel(z=slice(5)).rename({"z": "depthv"}))
        .mean("depthv")
    )
    vfull = (
        vds.vomecrty.isel(depthv=slice(10))
        .where(mesh.vmask.isel(z=slice(10)).rename({"z": "depthv"}))
        .mean("depthv")
    )
...
def _write_netcdf(src_dir, urot5, vrot5, urot10, vrot10, run_type):
    ...
    myds.VelEast5.attrs = {
        "long_name": "Upper 5 grid levels East Velocity",
    ...
    myds.VelNorth5.attrs = {
        "long_name": "Upper 5 grid levels North Velocity",
    ...
    myds.VelEast10.attrs = {
        "long_name": "Upper 10 grid levels East Velocity",
    ...
    myds.VelNorth10.attrs = {
        "long_name": "Upper 10 grid levels North Velocity",
    ...
    encoding = {
        "time_counter": {"units": "minutes since 1970-01-01 00:00", "dtype": float},
    ...
    myds.attrs = {
        ...
        "history": "CHS currents file made by nowcast worker: make_CHS_currents_file.py",
    }

    myds.coords["time_counter"].attrs = {
        "axis": "T",
        "comment": "time values are UTC at the centre of the intervals over which the "
