...
def main():
    # Prepare the worker
    base_parser = lib.basic_arg_parser(worker_name, description=__doc__, add_help=False)
...
def configure_argparser(prog, description, parents):
    ...
    parser.add_argument("host_name", help="Name of the host to upload the files to")
    parser.add_argument(
        "--run-date",
        type=lib.arrow_date,
        default=arrow.now(),
...
def upload_all_files(host_name, run_date, config):
    ...
    ssh_client, sftp_client = ssh_sftp.sftp(host_name, host["ssh key name"]["nowcast"])
    ...
    for day in range(-1, 2):
        ...
        localpath = os.path.join(config["weather"]["ops_dir"], dest_dir, filename)
    ...
    for day in range(-1, 2):
        ...
        remotepath = os.path.join(host["forcing"]["bc dir"], dest_dir, filename)
        upload_file(sftp_client, host_name, localpath, remotepath)

    # Restart File
    prev_run_id = run_date.shift(days=-1).date()
    prev_run_dir = prev_run_id.strftime("%d%b%y").lower()
    local_dir = os.path.join(config["run"]["results archive"]["nowcast"], prev_run_dir)
...
def make_remote_directory(sftp_client, host_name, remote_dir):
    sftp_client.mkdir(
        remote_dir, mode=int(lib.FilePerms(user="rwx", group="rwx", other="rx"))
    )
    logger.debug(
        "{remote} directory made on {host}".format(remote=remote_dir, host=host_name)
    )
